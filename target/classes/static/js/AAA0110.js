...
$(document).ready(function() {
	...
	$('#file').on('click', function(){
		$("#div3")
		.html( getMessage(CONFIRM_MESSAGE_ID, 'マンションアップロード'))
		.dialog({
			modal : true, // モーダル表示
			title : dialog.title,// タイトル
			buttons : { // ボタン
				"OK" : function() {
					var form = document.getElementsByName('uploadMansion');
					$(this).dialog("close");
					// valid file before submit
					var isValid = $("#uploadMansion").validationEngine('validate');
					//If valid
					if (isValid) {
...
function uploadFileImg(idInput, idFile) {
	$("#text-upload").prev(".formError").remove();
	idInput1 = "#" + idInput;
	idFile1 = "#" + idFile;
	$(idFile1).click();
}
...
function validateFileBeforeSubmit(form) {
	...
	// ファイルのアップロードサイズの上限チェック
	// ・ファイルのサイズをチェックする、ファイルのサイズは＞セッション情報のAAA0110_FILE_SIZE_MAXの場合はエラー。
